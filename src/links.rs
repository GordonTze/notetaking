...
impl LinkManager {
    ...
    pub fn add_link(&mut self, source: (usize, usize), target: (usize, usize)) {
    ...
    pub fn remove_link(&mut self, source: (usize, usize), target: (usize, usize)) {
    ...
    pub fn get_outgoing_links(&self, note: (usize, usize)) -> Vec<(usize, usize)> {
        self.outgoing_links.get(&note).cloned().unwrap_or_default()
    }

    pub fn get_backlinks(&self, note: (usize, usize)) -> Vec<(usize, usize)> {
        self.incoming_links.get(&note).cloned().unwrap_or_default()
    }

    pub fn scan_note_for_links(&mut self, content: &str, _source: (usize, usize)) -> Vec<String> {
