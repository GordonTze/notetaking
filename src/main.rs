        ...
        egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("✏ Edit", |ui| {
                    ...
                    ui.separator();
                    if ui.button(if self.is_favorite() { "⭐ Unfavorite" } else { "☆ Favorite" }).clicked() {
                ...
                ui.menu_button("👁 View", |ui| {
                    if ui.button(if self.show_markdown_preview { "📝 Edit Mode (Ctrl+P)" } else { "👁 Preview Mode (Ctrl+P)" }).clicked() {
                        if self.selected_note.is_some() {
                            self.show_markdown_preview = !self.show_markdown_preview;
                        }
                        ui.close_menu();
                ...
                ui.menu_button("⚙ Settings", |ui| {
                    ...
                    ui.separator();
                    if ui.checkbox(&mut self.auto_save_enabled, "💾 Auto-save").changed() {
                        println!("Auto-save: {}", self.auto_save_enabled);
                    }
                    if ui.checkbox(&mut self.autocomplete_enabled, "✨ Autocomplete").changed() {
                        self.autocomplete.set_enabled(self.autocomplete_enabled);
                        println!("Autocomplete: {}", self.autocomplete_enabled);
                    }
                    if ui.checkbox(&mut self.spellcheck_enabled, "✓ Spell Check").changed() {
